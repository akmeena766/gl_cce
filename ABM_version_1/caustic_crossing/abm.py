...
def abm_main(Dol, Dls, Dos, tEin_scale, source_plane_FOV, arc_angle, kappa_micro, ml_position, ml_mass):
    ...
    if input.plot_light_curve_parallel:
        ...
        light_curve = abm_light_curve(tEin_scale, Dol, Dls, Dos, path_vec, theta_boundary, beta_boundary_0, input.n_pixels, eta, 
                                  num_iter, delta_beta, arc_angle, kappa_micro, ml_position, ml_mass, 'parallel')
    ...
    if input.plot_light_curve_vertical:
        ...
        # Calculating the vertical light curve
        light_curve = abm_light_curve(tEin_scale, Dol, Dls, Dos, path_vec, theta_boundary, beta_boundary_0, input.n_pixels, eta, 
                                  num_iter, delta_beta, arc_angle, kappa_micro, ml_position, ml_mass, 'vertical')
...
def abm_light_curve(tEin_scale, Dol, Dls, Dos, path_vec, theta_lim_0, beta_lim_0, n_pixels, eta, num_iter, delta_beta, arc_angle, kappa_micro, ml_position, ml_mass, lcd):
        ...
        for i in range(0, int(num_iter) + 1):
            if i < int(num_iter):
                ...
                global_theta = split_and_identify(tEin_scale, Dol, Dls, Dos, new_theta_boundaries, all_thetas, new_beta_boundaries, 
                                                  path_vec1, n_pixels, arc_angle, kappa_micro, ml_position, ml_mass, lcd, 'global')                                        

                if len(global_theta) == 0:
                   global_beta = np.array([])
                   print('No relevant pixels found in image plane. \n Maybe something went wrong.') 
                   break
                ...
                global_beta = split_and_identify(tEin_scale, Dol, Dls, Dos, new_theta_boundaries, all_thetas, new_beta_boundaries, 
                                                 path_vec1, n_pixels, arc_angle, kappa_micro, ml_position, ml_mass, lcd, 'final')
...
def split_and_identify(tEin_scale, Dol, Dls, Dos, theta_lim, theta_offset, beta_lim, beta_offset, n_pixels, arc_angle, kappa_micro, ml_position, ml_mass, lcd, itrv):
    num_cores = multiprocessing.cpu_count()  
    ...
    if n_runs > 1:
        ...
        (delayed(hf.img2src)(tEin_scale, Dol, Dls, Dos, theta_pixels[i*l_tmp : (i+1)*l_tmp], arc_angle, kappa_micro, ml_position, ml_mass) for i in range(n_runs-1))
        
        tmp_beta_pixels.append(hf.img2src(tEin_scale, Dol, Dls, Dos, theta_pixels[(n_runs-1)*l_tmp : ], arc_angle, kappa_micro, ml_position, ml_mass))
        
        beta_pixels = np.concatenate(tmp_beta_pixels)
    else:
        beta_pixels = hf.img2src(tEin_scale, Dol, Dls, Dos, theta_pixels, arc_angle, kappa_micro, ml_position, ml_mass)    
