...
def plot_critical(tEin_scale, theta_x, theta_y, mu_lens, ml_position, ml_mass, theta_boundary):
    ...
    cs1 = ax.imshow(abs(mu_lens), origin = 'lower', extent = (xmin, xmax, ymin, ymax), cmap=cmap, norm=mpl.colors.LogNorm(vmin=1e2, vmax=1e6), aspect='auto')
...
def plot_caustics(tEin_scale, beta_x, beta_y, mu_source, source_plane_FOV):
    ...
    cs1 = ax.imshow(mu_source + 1, origin = 'lower', extent = (xmin, xmax, ymin, ymax), cmap=cmap, norm=mpl.colors.LogNorm(vmin=1e2, vmax=1e6), aspect='auto')
...
def parameters(ml_position, ml_mass, tEin_scale, kappa_micro):
    ...
    file.write('kappa_micro = %3.4f\n' % kappa_micro)
    file.write('Stellar density = %5.4f (in MASS_SUN/PC^2)\n' % input.stellar_density)
