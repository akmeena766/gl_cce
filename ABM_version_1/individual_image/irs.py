...
def irs_main(Dol, Dls, Dos, tEin_scale, source_plane_FOV, ml_position, ml_mass):
    ...
    source_plane_FOV = [source_plane_FOV, source_plane_FOV]/tEin_scale
    
    
    #### Critical curve generation in the lens plane
    if input.plot_critical:
        theta_x, theta_y, mu_lens = hf.critical_curves(tEin_scale, Dol, Dls, Dos, theta_boundary, ml_position, ml_mass)
    ...
    if input.plot_caustics == True:
        beta_x, beta_y, mu_source = irs_caustics_random_shooting(tEin_scale, Dol, Dls, Dos, theta_boundary, source_plane_FOV, ml_position, ml_mass)
...
def irs_caustics_random_shooting(tEin_scale, Dol, Dls, Dos, theta_boundary, source_plane_FOV, ml_position, ml_mass):
    ...
    mu_grid_temp_array = Parallel(n_jobs=num_cores, require='sharedmem')\
        (delayed(parallel_irs)(i, num_of_rays, n_runs, start_time, mu_grid, l_tmp, area_per_ray, source_plane_FOV, 
         tEin_scale, Dol, Dls, Dos, theta_boundary, ml_position, ml_mass) for i in range(n_runs))
    ...
    if n_runs * l_tmp < num_of_rays:  # if some values are left
        ...
        beta = hf.img2src(tEin_scale, Dol, Dls, Dos, theta, ml_position, ml_mass)
...
def parallel_irs(i, num_of_rays, n_runs, start_time, mu_grid, l_tmp, area_per_ray, source_plane_FOV, 
                 tEin_scale, Dol, Dls, Dos, theta_boundary, ml_position, ml_mass):
