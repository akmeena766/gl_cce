...
#### Critical density
def stellar_density(Dol, critDen):
...
def generate_microlens_population(image_plane_FOV, critDen):
    lower_limit = input.lower_limit
    upper_limit = input.upper_limit
    alpha = input.alpha
    
    kappa_micro = abs(input.kappa_micro)				# Microlens convergence
...
def magnification_function(i,theta_x, theta_y_value, tEin_scale, Dol, Dls, Dos, ml_x, ml_y, ml_mass):
    ...
    ks, g1s, g2s = input.kappa_smooth, input.gamma_smooth, 0
    
    mag = (1. - ks)**2 - (g1s + gamma1)**2 - (g2s + gamma2)**2
    return(1/mag)


#### Calculating the image plane grid magnification values (only for IRS)
def critical_curves(tEin_scale, Dol, Dls, Dos, theta_boundary, ml_position, ml_mass):
    ...
    (delayed(magnification_function)(i, theta_x, theta_y[i,0], tEin_scale, Dol, Dls, Dos, ml_x, ml_y, ml_mass) for i in range(theta_x.shape[0]))
...
def img2src(tEin_scale, Dol, Dls, Dos, theta, ml_position, ml_mass):
    n = ml_mass.shape[0]
    l = theta.shape[0]
    ...
    beta_x = theta_x*(1- ks - g1s) - g2s*theta_y - alpha_x
    beta_y = theta_y*(1- ks + g1s) - g2s*theta_x - alpha_y
...
def mag_binning(beta, weight, boundary, beta_res = input.beta_res, beta_offset=[0,0]):
    ...
    min_y, max_y = -boundary[0]/2 + beta_offset[1], boundary[1]/2 + beta_offset[1]
